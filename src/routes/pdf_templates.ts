...
router.get('/', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM pdf_templates');
...
router.get('/:id', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM pdf_templates WHERE id = ?', [req.params.id]);
    const rows = result[0] as any[];
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await pool.query('INSERT INTO pdf_templates SET ?', [transformedData]);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query('UPDATE pdf_templates SET ? WHERE id = ?', [transformedData, req.params.id]);
