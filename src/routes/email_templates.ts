...
router.get('/:id', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM email_templates WHERE id = ?', [req.params.id]);
    const rows = result[0] as any[];
...
router.post('/', async (req, res) => {
  try {
    const data = req.body;
    const result = await pool.query('INSERT INTO email_templates SET ?', [data]);
...
router.put('/:id', async (req, res) => {
  try {
    const data = req.body;
    const result = await pool.query('UPDATE email_templates SET ? WHERE id = ?', [data, req.params.id]);
