...
router.get('/:id', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM form_fields WHERE id = ?', [req.params.id]);
    const rows = result[0] as any[];
...
router.post('/', async (req, res) => {
  try {
    const data = req.body;
    const result = await pool.query('INSERT INTO form_fields SET ?', [data]);
...
router.put('/:id', async (req, res) => {
  try {
    const data = req.body;
    const result = await pool.query('UPDATE form_fields SET ? WHERE id = ?', [data, req.params.id]);
