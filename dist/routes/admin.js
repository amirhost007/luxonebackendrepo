...
const storage = multer_1.default.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        cb(null, 'company-logo-' + uniqueSuffix + path_1.default.extname(file.originalname));
    }
});
...
router.get('/company-settings', async (req, res) => {
    try {
        const [rows] = await db_1.pool.query('SELECT * FROM company_settings WHERE id = 1');
        const settings = rows;
        res.json(settings[0] || {});
    }
    catch (err) {
...
router.post('/company-logo', upload.single('logo'), async (req, res) => {
    try {
        ...
        const [result] = await db_1.pool.query('UPDATE company_settings SET logo_file_name = ?, logo_file_path = ?, logo_file_size = ?, logo_mime_type = ? WHERE id = 1', [file.originalname, filePath, file.size, file.mimetype]);
        res.json({
            success: true,
            file: {
...
router.put('/company-settings', async (req, res) => {
    try {
        ...
        const [result] = await db_1.pool.query('UPDATE company_settings SET ? WHERE id = 1', [dbData]);
...
router.get('/form-fields', async (req, res) => {
    try {
        const [rows] = await db_1.pool.query('SELECT * FROM form_fields ORDER BY step_number, display_order');
        // Map database fields to frontend format
        const mappedFields = rows.map(field => ({
            id: field.field_id,
...
router.post('/form-fields', async (req, res) => {
    try {
        ...
        const [result] = await db_1.pool.query('INSERT INTO form_fields SET ?', [dbData]);
...
router.put('/form-fields/:id', async (req, res) => {
    try {
        ...
        const [result] = await db_1.pool.query('UPDATE form_fields SET ? WHERE id = ?', [dbData, req.params.id]);
        res.json({ success: true, affectedRows: result.affectedRows });
    }
    catch (err) {
        res.status(500).json({ error: 'Database error', details: err });
...
router.delete('/form-fields/:id', async (req, res) => {
    try {
        const [result] = await db_1.pool.query('DELETE FROM form_fields WHERE id = ?', [req.params.id]);
        res.json({ success: true, affectedRows: result.affectedRows });
    }
    catch (err) {
        res.status(500).json({ error: 'Database error', details: err });
...
router.get('/pdf-templates', async (req, res) => {
    try {
        const [rows] = await db_1.pool.query('SELECT * FROM pdf_templates ORDER BY is_default DESC, name');
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: 'Database error', details: err });
...
router.post('/pdf-templates', async (req, res) => {
    try {
        const data = req.body;
        const [result] = await db_1.pool.query('INSERT INTO pdf_templates SET ?', [data]);
        res.status(201).json({ id: result.insertId });
    }
    catch (err) {
        res.status(500).json({ error: 'Database error', details: err });
...
router.put('/pdf-templates/:id', async (req, res) => {
    try {
        const data = req.body;
        const [result] = await db_1.pool.query('UPDATE pdf_templates SET ? WHERE id = ?', [data, req.params.id]);
        res.json({ success: true, affectedRows: result.affectedRows });
    }
    catch (err) {
        res.status(500).json({ error: 'Database error', details: err });
...
router.get('/pricing-rules', async (req, res) => {
    try {
        const [rows] = await db_1.pool.query('SELECT * FROM pricing_rules ORDER BY rule_name');
        // Map database fields to frontend format
        const mappedRules = rows.map(rule => ({
            id: rule.id.toString(),
...
router.post('/pricing-rules', async (req, res) => {
    try {
        ...
        const [result] = await db_1.pool.query('INSERT INTO pricing_rules SET ?', [dbData]);
        res.status(201).json({ id: result.insertId });
    }
    catch (err) {
        res.status(500).json({ error: 'Database error', details: err });
...
router.put('/pricing-rules/:id', async (req, res) => {
    try {
        ...
        const [result] = await db_1.pool.query('UPDATE pricing_rules SET ? WHERE id = ?', [dbData, req.params.id]);
        res.json({ success: true, affectedRows: result.affectedRows });
    }
    catch (err) {
        res.status(500).json({ error: 'Database error', details: err });
...
router.get('/analytics', async (req, res) => {
    try {
        const [quotationStats] = await db_1.pool.query(`
      SELECT 
      ...
      FROM quotations
    `);
        const [monthlyStats] = await db_1.pool.query(`
      SELECT 
